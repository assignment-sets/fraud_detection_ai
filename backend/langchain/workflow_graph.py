...
class State(TypedDict):
    ...
    is_fake_news: Optional[
        bool
    ]  # Result of news verification (True=fake, False=legitimate, None=not checked)
    news_verification_requested: Optional[
        bool
    ]  # Flag to track if news verification has been requested
    is_irrelevant_input: Optional[
        bool
    ]  # Flag to track if the input is irrelevant to fraud detection
...
SYSTEM_PROMPT = llm_instance.get_system_prompt()
...
def llm_node(state: State) -> State:
    ...
    if len(messages) == 1:  # Only system message exists
        ...
        if extracted_urls:
            ...
            state["list_of_actions"].append(
                f"Extracted {len(extracted_urls)} URLs from user query"
            )
    ...
    # Check if news has been fetched and needs verification
        ...
        "fetched_news" in state
        and state["fetched_news"]
        and (state["is_fake_news"] is None)
        and not state["news_verification_requested"]
    ):
        ...
        try:
            ...
            state["list_of_actions"].append(
                "Extracted final reasoning summary from news verification"
            )
            ...
            if (
                ...
                or "no evidence" in content
            ):
                state["is_fake_news"] = True
                state["list_of_actions"].append(
                    "Determined news is likely fake based on article analysis"
                )
            elif (
                "legitimate" in content
                or "supported" in content
                or "confirmed" in content
                or "real" in content
            ):
                state["is_fake_news"] = False
                state["list_of_actions"].append(
                    "Determined news is likely legitimate based on article analysis"
                )
            else:
                # If unclear, default to considering it suspicious
                state["is_fake_news"] = True
                state["list_of_actions"].append(
                    "News verification inconclusive - treating as potentially fake"
                )
            ...
            messages.append(
                AIMessage(
                    content="I encountered an error while verifying the news. Treating it as potentially suspicious."
                )
            )
            state["is_fake_news"] = True
            state["list_of_actions"].append(f"Error during news verification: {str(e)}")
            state["final_reasoning_summary"] = (
                "Error during news verification. Treating as potentially suspicious."
            )
            return {**state, "messages": messages}

    # If this message follows tool execution, add appropriate context
    elif len(messages) > 1 and any(
    ...
    if not (
        ...
        try:
            ...
            if "input_types" in state and not state["input_types"]:
                ...
                is_irrelevant = any(
                    indicator in content for indicator in irrelevant_indicators
                )
                ...
                if is_irrelevant:
                    ...
                    state["list_of_actions"].append(
                        "Detected irrelevant input not related to fraud detection"
                    )
                    ...
                    state["list_of_actions"].append(
                        "Set final reasoning summary for irrelevant input"
                    )

                elif detected_types:
                    state["input_types"] = detected_types
                    state["list_of_actions"].append(
                        f"Detected input types: {', '.join(detected_types)}"
                    )
            ...
            if (
                not getattr(ai_response, "tool_calls", [])
                and not state["final_reasoning_summary"]
            ):
                content = ai_response.content.lower()
                ...
                if "final conclusion" in content:
                    ...
                    state["final_reasoning_summary"] = final_conclusion
                    state["list_of_actions"].append(
                        "Extracted final reasoning summary from conclusion section"
                    )

                    # Check if we need to update any fraud flags based on the conclusion
                    if (
                        "url" in state.get("input_types", [])
                        and state["is_fraud_url"] is None
                    ):
                        if (
                            "fraud" in final_conclusion
                            or "malicious" in final_conclusion
                            or "phishing" in final_conclusion
                        ):
                            state["is_fraud_url"] = True
                            state["list_of_actions"].append(
                                "Updated is_fraud_url to True based on conclusion"
                            )
                        elif (
                            "safe" in final_conclusion
                            or "legitimate" in final_conclusion
                        ):
                            state["is_fraud_url"] = False
                            state["list_of_actions"].append(
                                "Updated is_fraud_url to False based on conclusion"
                            )

                    if (
                        "sms" in state.get("input_types", [])
                        and state["is_fraud_sms"] is None
                    ):
                        if "fraud" in final_conclusion or "scam" in final_conclusion:
                            state["is_fraud_sms"] = True
                            state["list_of_actions"].append(
                                "Updated is_fraud_sms to True based on conclusion"
                            )
                        elif (
                            "safe" in final_conclusion
                            or "legitimate" in final_conclusion
                            ...
                            state["list_of_actions"].append(
                                "Updated is_fraud_sms to False based on conclusion"
                            )

                    if (
                        "email" in state.get("input_types", [])
                        and state["is_fraud_email"] is None
                    ):
                        if (
                            "fraud" in final_conclusion
                            or "phishing" in final_conclusion
                            or "scam" in final_conclusion
                        ):
                            state["is_fraud_email"] = True
                            state["list_of_actions"].append(
                                "Updated is_fraud_email to True based on conclusion"
                            )
                        elif (
                            "safe" in final_conclusion
                            or "legitimate" in final_conclusion
                            ...
                            state["list_of_actions"].append(
                                "Updated is_fraud_email to False based on conclusion"
                            )
            ...
            messages.append(
                AIMessage(
                    content="I encountered an error while analyzing your query. Please try again."
                )
            )
            state["final_reasoning_summary"] = (
...
def tool_node(state: State) -> State:
    ...
    for tool_call in tool_calls:
        ...
        if tool_signature in state["executed_tools"]:
            results.append(
                ToolMessage(
                    content="Note: This exact tool call has already been executed. Using previous results.",
...
def should_continue(state: State) -> Literal["tool_node", "llm", END]:  # type: ignore
    ...
    if (
        "fetched_news" in state
        and state["fetched_news"]
        and state.get("is_fake_news") is None
        and not state.get("news_verification_requested", False)
    ):
        return "llm"
    ...
    if (
        "news" in state.get("input_types", [])
        and state.get("is_fake_news") is None
        and "fetched_news" in state
        and state["fetched_news"]
    ):
