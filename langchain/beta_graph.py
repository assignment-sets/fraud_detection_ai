...
def llm_node(state: State) -> State:
    ...
    # Check if news has been fetched and needs verification
    elif "fetched_news" in state and (state["is_fake_news"] is None) and not state["news_verification_requested"]:
        ...
        # Check if no news was fetched - mark as fake news
        if not state["fetched_news"]:
            state["is_fake_news"] = True
            state["list_of_actions"].append("No news articles found - marked as fake news")
            state["final_reasoning_summary"] = "No reputable news sources found reporting this information. This suggests the news is likely fake or unverified."
            messages.append(AIMessage(content=f"""
            I've searched for news articles related to your query but couldn't find any relevant information from reputable sources. 
            
            When no information can be found about a news claim from legitimate news sources, this is often an indicator that the information may be:
            - Fake or fabricated news
            - A very recent development not yet reported
            - An obscure topic not widely covered
            - Misinformation or disinformation
            
            FINAL CONCLUSION: No reputable news sources found reporting this information. This suggests the news is likely fake or unverified.
            """))
    ...
    if not (("fetched_news" in state and state["is_fake_news"] is None and not state["news_verification_requested"])):
        try:
            ...
            if "input_types" in state and not state["input_types"]:
                ...
                if is_irrelevant:
                    ...
                    # If no types detected but not marked irrelevant, mark as irrelevant
                    state["is_irrelevant_input"] = True
                    state["input_types"] = ["irrelevant"]
                    state["list_of_actions"].append("No valid input types detected - marked as irrelevant")
                    state["final_reasoning_summary"] = "This input does not contain any recognizable email, SMS, URL, or news content for fraud analysis."
...
if __name__ == '__main__':
    try:
        initial_state = {
            "user_query": r"""do you wanna fuck
            """,
            "input_types": [],
            "messages": [],
            "available_tools": [
